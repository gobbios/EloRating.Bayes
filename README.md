...
absolute sense (but far from perfect). The model that assumes a fixed SD
for the start ratings doesn’t recover the true start ratings in an
absolute sense. So that looks like a good start. TBC.
